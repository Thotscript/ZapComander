async function processText(sessionName, message) {
    try {
      ...
      const convoKey = `${sessionName}:${message.from}`; // <- distinção por usuário
...
Agradeça a participação e reforce que os dados foram compilados com sucesso.
      ...
      do {
        ...
        const runStatus = await openai.beta.threads.runs.retrieve(conversation.thread_id, run.id);
      ...
      console.log(`🤖 [${sessionName}] ${message.from}: ${responseText}`);
  
      await client.sendText(message.from, responseText);
  
    } catch (error) {
      console.error(`❌ Erro em processText:`, error.response?.data || error.message);
    }
  }